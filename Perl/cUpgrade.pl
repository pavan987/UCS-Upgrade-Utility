...
my $buildname = "ucs-k9-bundle-c-series.$ver.$buildnum.C.$buildtype";
...
{
print "File Exists\n";
mydb::insertLog($DB_user, "C-Upgrade", "Download Complete:","  Bundle $buildname already Present in Local server");
}
else {

print "Downloading $buildname to local server from build server\n";
mydb::insertLog($DB_user, "C-Upgrade", "Download Started:","  Downloading $buildname to local server from build server");
system("cp $path /var/www/html/perl/builds/$branch/");
mydb::insertLog($DB_user, "C-Upgrade", "Download Completed","  $buildname Downloaded to local server");
...
$exp->send( "nbv12345\r");
$exp->expect($timeout,"#");
mydb::insertLog($DB_user, "C-Upgrade", "Downloading to UCSM:","  Downloading $buildname to UCSM");
...
mydb::insertLog($DB_user, "C-Upgrade", "Download Completed:","  $buildname Downloaded to UCSM");
...
$exp->expect($timeout,"#");
$exp->send("set rack-vers $package");
$exp->expect($timeout,"#");
$exp->send("commit-buffer\r");
$exp->expect($timeout,"#");

print "Auto Install Upgrade Process Started\n";
mydb::insertLog($DB_user, "C-Upgrade", "Upgrade Started:","  Upgrading Endpoints to build $buildnum using Host Fw pack");
$exp->send("exit\r");
$exp->close();
...
{
 print("Script FAIL\n");
 mydb::insertLog($DB_user, "C-Upgrade", $loghead,$logmesg);
}
