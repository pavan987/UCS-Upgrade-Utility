...
#Read Inputs
if((!defined $ARGV[0]) ) {
print "Usage: ./upgrade.pl Username Branch buildnum buildtype granada\n";
exit;
}
my $DB_user = $ARGV[0];
my $branch = $ARGV[1];
...
$ver = "2.5.1";
$start ="ucs-k9-bundle-infra";
}

elsif(($branch eq "granada-builds") || ( $branch eq "granada_bw-builds")) {

if($granada eq "3gfi") {
$start="ucs-6300-k9-bundle-infra";
}
elsif($branch eq "mini"){
$start="ucs-mini-k9-bundle-infra";
}
else {
$start="ucs-k9-bundle-infra";
}
...
my $buildname = "$start.$ver.$buildnum.A.$buildtype";
print $buildname;
# Builds Directory
...
{
print "File Exists\n";
mydb::insertLog($DB_user, "A-Upgrade", "Download Complete:","  Bundle $buildname already Present in Local server");
}
else {

print "Downloading $buildname to local server from build server\n";
mydb::insertLog($DB_user, "A-Upgrade", "Download Started:","  Downloading $buildname to local server from build server");
system("cp $path /var/www/html/perl/builds/$branch/");
mydb::insertLog($DB_user, "A-Upgrade", "Download Completed","  $buildname Downloaded to local server");
...
$exp->send( "nbv12345\r");
$exp->expect($timeout,"#");
mydb::insertLog($DB_user, "A-Upgrade", "Downloading to UCSM:","  Downloading $buildname to UCSM");
...
mydb::insertLog($DB_user, "A-Upgrade", "Download Completed:","  $buildname Downloaded to UCSM");
...
#Triger Autoinstall
$exp->send("scope auto-install\r");
$exp->expect($timeout,"#");
$exp->send("install infra infra-vers $package");
$exp->expect($timeout,"yes");
$exp->send("yes\r");
$exp->expect($timeout,"#");
print "Auto Install Upgrade Process Started\n";
mydb::insertLog($DB_user, "A-Upgrade", "Upgrade Started:","  Upgrading UCSM to build $buildnum using autoinstall");
$exp->send("acknowledge primary fabric-interconnect reboot\r");
$exp->expect($timeout,"#");
...
{
 print("Script FAIL\n");
 mydb::insertLog($DB_user, "A-Upgrade", $loghead,$logmesg);
}
